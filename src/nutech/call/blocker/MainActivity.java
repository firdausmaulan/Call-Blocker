package nutech.call.blocker;

import java.util.List;
import nutech.call.blocker.R;
...
public class MainActivity extends ActionBarActivity implements OnClickListener,
	...
	// This holds the value of the row number, which user has selected for
	// further action
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Initialization of the listview of the Main screen to display black
		...
		final View rowView = inflater.inflate(R.layout.list_item, listview,
				false);
		listview.addHeaderView(rowView);

		// Attach OnItemLongClickListener to track user action and perform
		// accordingly
		listview.setOnItemLongClickListener(this);
	}

	private void populateNoRecordMsg() {
		// If, no record found in the database, appropriate message needs to be
		// displayed.
		if (blockList.size() == 0) {
	...
	protected void onResume() {
		...
		if (listview.getChildCount() > 1)
			listview.removeFooterView(listview.getChildAt(listview
					.getChildCount() - 1));

		// Now, link the CustomArrayAdapter with the ListView
		listview.setAdapter(new CustomArrayAdapter(this, R.layout.list_item,
				blockList));
	...
	private void showDialog() {
		// Before deletion of the long pressed record, need to confirm with the
		// user. So, build the AlartBox first
		final AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
				this);

		// Set the appropriate message into it.
		alertDialogBuilder
				.setMessage("Are you Really want to delete the selected record ?");

		// Add a positive button and it's action. In our case action would be
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface arg0, int arg1) {
						try {
							...
							blackListDao.delete(blockList
									.get(selectedRecordPosition));

							// Removing the same from the List to remove from
							// display as well
		...
		// Add a negative button and it's action. In our case, just hide the
		// dialog box
	...
	public class CustomArrayAdapter extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			((TextView) convertView.findViewById(R.id.serial_tv)).setText(""
					+ (position + 1));
			((TextView) convertView.findViewById(R.id.phone_number_tv))
